...
#             # Print keys for the first file to help debug
#             if len(hrirs_list) == 0:
#                 print(f"First MAT file contains keys: {list(data.keys())}")
            
#             # Find the HRIR data in the MAT file
#             # Try common keys for HRIR data
#             hrir_data = None
#             possible_keys = ['hrir', 'h_data', 'IR', 'impulse_response', 'h', 'data']
...
#                 # If we can't find it by name, look for the largest array that might be the HRIR
#                 largest_array = None
#                 largest_size = 0
...
#     # Most common pattern is (2, n_samples) where dimension 0 is [left, right]
#     # Check the shape of the first HRIR
#     first_hrir = hrirs_list[0]
#     print(f"HRIR shape: {first_hrir.shape}")
...
#     azimuth_diffs = np.abs(azimuths - target_azimuth)
    
#     # Normalize differences to give equal weight
#     if len(distances) > 1 and np.max(distance_diffs) > 0:
#         distance_diffs = distance_diffs / np.max(distance_diffs)
#     if len(elevations) > 1 and np.max(elevation_diffs) > 0:
#         elevation_diffs = elevation_diffs / np.max(elevation_diffs)
#     if len(azimuths) > 1 and np.max(azimuth_diffs) > 0:
#         azimuth_diffs = azimuth_diffs / np.max(azimuth_diffs)
    
#     # Combined distance metric
#     combined_diffs = distance_diffs * 3 + elevation_diffs * 2 + azimuth_diffs
...
#     # Check the shape of the HRIR
#     if hrir.shape[1] == 8:  # If it's in the (4800, 8) format
#         # Assuming channels 0 and 1 are left and right ears
#         left_hrir = hrir[:, 0]
#         right_hrir = hrir[:, 1]
#     elif hrir.shape[0] == 2:  # If it's in the (2, samples) format
#         left_hrir = hrir[0]
#         right_hrir = hrir[1]
#     else:
#         print(f"Warning: Unexpected HRIR shape: {hrir.shape}")
#         # Default to using the first two channels if available
#         if hrir.shape[1] >= 2:
#             left_hrir = hrir[:, 0]
#             right_hrir = hrir[:, 1]
#         else:
#             # Fallback to using the same channel for both ears
#             left_hrir = right_hrir = hrir[:, 0] if len(hrir.shape) > 1 else hrir
